...
pub fn validate_repo_path_safe(
    ...
    validate_repo_path(org, project, repo).map_err(|err| GitError::Sandbox(err.to_string()))?;

    // Construct and validate the full path
    let repo_path = repos_base
        .join(org)
        .join(project)
...
pub fn list_commits(repo_path: &Path, max_count: usize) -> Result<Vec<CommitInfo>, GitError> {
    ...
    revwalk.push(
        head.target()
            .ok_or_else(|| GitError::InvalidRef("HEAD has no target".to_string()))?,
    )?;
...
fn remove_at_path(
    ...
    if path_parts.len() == 1 {
        ...
        let entry = current_tree
            .get_name(dir_name)
            .ok_or_else(|| GitError::NotFound(format!("Directory not found: {}", dir_name)))?;
...
pub fn get_branch_diff(
    ...
    for i in 0..diff.deltas().len() {
        let delta = diff
            .get_delta(i)
            .ok_or_else(|| GitError::NotFound("Delta not found".to_string()))?;
        ...
        diff.print(DiffFormat::Patch, |delta2, _hunk, line| {
            if let Some(p) = delta2
                .new_file()
                .path()
                .or_else(|| delta2.old_file().path())
            {
    ...
    pub fn finish(&self) -> Result<Vec<u8>, GitError> {
        if self.service == "git-upload-pack" {
            ...
            if !self.input_buffer.is_empty() {
                let pack_response =
                    process_upload_pack_request(&self.repo_path, &self.input_buffer)?;
