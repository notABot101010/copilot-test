...
pub unsafe fn chacha_blocks_avx2_x8<const ROUNDS: usize>(
    ...
    let mut v12 = _mm256_setr_epi32(
        (counter_base) as i32, (counter_base + 1) as i32,
        (counter_base + 2) as i32, (counter_base + 3) as i32,
        (counter_base + 4) as i32, (counter_base + 5) as i32,
        (counter_base + 6) as i32, (counter_base + 7) as i32,
    );
    let mut v13 = _mm256_setr_epi32(
        (counter_base >> 32) as i32, ((counter_base + 1) >> 32) as i32,
        ((counter_base + 2) >> 32) as i32, ((counter_base + 3) >> 32) as i32,
        ((counter_base + 4) >> 32) as i32, ((counter_base + 5) >> 32) as i32,
        ((counter_base + 6) >> 32) as i32, ((counter_base + 7) >> 32) as i32,
    );
    let mut v14 = _mm256_set1_epi32(state[14] as i32);
    let mut v15 = _mm256_set1_epi32(state[15] as i32);
    
    let orig0 = v0; let orig1 = v1; let orig2 = v2; let orig3 = v3;
    let orig4 = v4; let orig5 = v5; let orig6 = v6; let orig7 = v7;
    let orig8 = v8; let orig9 = v9; let orig10 = v10; let orig11 = v11;
    ...
    // Add original state
    v0 = _mm256_add_epi32(v0, orig0); v1 = _mm256_add_epi32(v1, orig1);
    v2 = _mm256_add_epi32(v2, orig2); v3 = _mm256_add_epi32(v3, orig3);
    v4 = _mm256_add_epi32(v4, orig4); v5 = _mm256_add_epi32(v5, orig5);
    v6 = _mm256_add_epi32(v6, orig6); v7 = _mm256_add_epi32(v7, orig7);
    v8 = _mm256_add_epi32(v8, orig8); v9 = _mm256_add_epi32(v9, orig9);
    v10 = _mm256_add_epi32(v10, orig10); v11 = _mm256_add_epi32(v11, orig11);
    v12 = _mm256_add_epi32(v12, orig12); v13 = _mm256_add_epi32(v13, orig13);
    v14 = _mm256_add_epi32(v14, orig14); v15 = _mm256_add_epi32(v15, orig15);
    
    // Extract and transpose low 128-bit lanes (blocks 0-3)
    let v0_lo = _mm256_castsi256_si128(v0); let v0_hi = _mm256_extracti128_si256::<1>(v0);
    let v1_lo = _mm256_castsi256_si128(v1); let v1_hi = _mm256_extracti128_si256::<1>(v1);
    let v2_lo = _mm256_castsi256_si128(v2); let v2_hi = _mm256_extracti128_si256::<1>(v2);
    let v3_lo = _mm256_castsi256_si128(v3); let v3_hi = _mm256_extracti128_si256::<1>(v3);
    let v4_lo = _mm256_castsi256_si128(v4); let v4_hi = _mm256_extracti128_si256::<1>(v4);
    let v5_lo = _mm256_castsi256_si128(v5); let v5_hi = _mm256_extracti128_si256::<1>(v5);
    let v6_lo = _mm256_castsi256_si128(v6); let v6_hi = _mm256_extracti128_si256::<1>(v6);
    let v7_lo = _mm256_castsi256_si128(v7); let v7_hi = _mm256_extracti128_si256::<1>(v7);
    let v8_lo = _mm256_castsi256_si128(v8); let v8_hi = _mm256_extracti128_si256::<1>(v8);
    let v9_lo = _mm256_castsi256_si128(v9); let v9_hi = _mm256_extracti128_si256::<1>(v9);
    let v10_lo = _mm256_castsi256_si128(v10); let v10_hi = _mm256_extracti128_si256::<1>(v10);
    let v11_lo = _mm256_castsi256_si128(v11); let v11_hi = _mm256_extracti128_si256::<1>(v11);
    let v12_lo = _mm256_castsi256_si128(v12); let v12_hi = _mm256_extracti128_si256::<1>(v12);
    let v13_lo = _mm256_castsi256_si128(v13); let v13_hi = _mm256_extracti128_si256::<1>(v13);
    let v14_lo = _mm256_castsi256_si128(v14); let v14_hi = _mm256_extracti128_si256::<1>(v14);
    let v15_lo = _mm256_castsi256_si128(v15); let v15_hi = _mm256_extracti128_si256::<1>(v15);
    
    // Transpose blocks 0-3 from low halves
    let t0 = _mm_unpacklo_epi32(v0_lo, v1_lo); let t1 = _mm_unpackhi_epi32(v0_lo, v1_lo);
    let t2 = _mm_unpacklo_epi32(v2_lo, v3_lo); let t3 = _mm_unpackhi_epi32(v2_lo, v3_lo);
    let r0 = _mm_unpacklo_epi64(t0, t2); let r1 = _mm_unpackhi_epi64(t0, t2);
    let r2 = _mm_unpacklo_epi64(t1, t3); let r3 = _mm_unpackhi_epi64(t1, t3);
    let t4 = _mm_unpacklo_epi32(v4_lo, v5_lo); let t5 = _mm_unpackhi_epi32(v4_lo, v5_lo);
    let t6 = _mm_unpacklo_epi32(v6_lo, v7_lo); let t7 = _mm_unpackhi_epi32(v6_lo, v7_lo);
    let r4 = _mm_unpacklo_epi64(t4, t6); let r5 = _mm_unpackhi_epi64(t4, t6);
    let r6 = _mm_unpacklo_epi64(t5, t7); let r7 = _mm_unpackhi_epi64(t5, t7);
    let t8 = _mm_unpacklo_epi32(v8_lo, v9_lo); let t9 = _mm_unpackhi_epi32(v8_lo, v9_lo);
    let t10 = _mm_unpacklo_epi32(v10_lo, v11_lo); let t11 = _mm_unpackhi_epi32(v10_lo, v11_lo);
    let r8 = _mm_unpacklo_epi64(t8, t10); let r9 = _mm_unpackhi_epi64(t8, t10);
    let r10 = _mm_unpacklo_epi64(t9, t11); let r11 = _mm_unpackhi_epi64(t9, t11);
    let t12 = _mm_unpacklo_epi32(v12_lo, v13_lo); let t13 = _mm_unpackhi_epi32(v12_lo, v13_lo);
    let t14 = _mm_unpacklo_epi32(v14_lo, v15_lo); let t15 = _mm_unpackhi_epi32(v14_lo, v15_lo);
    let r12 = _mm_unpacklo_epi64(t12, t14); let r13 = _mm_unpackhi_epi64(t12, t14);
    let r14 = _mm_unpacklo_epi64(t13, t15); let r15 = _mm_unpackhi_epi64(t13, t15);
    ...
    let t0 = _mm_unpacklo_epi32(v0_hi, v1_hi); let t1 = _mm_unpackhi_epi32(v0_hi, v1_hi);
    let t2 = _mm_unpacklo_epi32(v2_hi, v3_hi); let t3 = _mm_unpackhi_epi32(v2_hi, v3_hi);
    let r0 = _mm_unpacklo_epi64(t0, t2); let r1 = _mm_unpackhi_epi64(t0, t2);
    let r2 = _mm_unpacklo_epi64(t1, t3); let r3 = _mm_unpackhi_epi64(t1, t3);
    let t4 = _mm_unpacklo_epi32(v4_hi, v5_hi); let t5 = _mm_unpackhi_epi32(v4_hi, v5_hi);
    let t6 = _mm_unpacklo_epi32(v6_hi, v7_hi); let t7 = _mm_unpackhi_epi32(v6_hi, v7_hi);
    let r4 = _mm_unpacklo_epi64(t4, t6); let r5 = _mm_unpackhi_epi64(t4, t6);
    let r6 = _mm_unpacklo_epi64(t5, t7); let r7 = _mm_unpackhi_epi64(t5, t7);
    let t8 = _mm_unpacklo_epi32(v8_hi, v9_hi); let t9 = _mm_unpackhi_epi32(v8_hi, v9_hi);
    let t10 = _mm_unpacklo_epi32(v10_hi, v11_hi); let t11 = _mm_unpackhi_epi32(v10_hi, v11_hi);
    let r8 = _mm_unpacklo_epi64(t8, t10); let r9 = _mm_unpackhi_epi64(t8, t10);
    let r10 = _mm_unpacklo_epi64(t9, t11); let r11 = _mm_unpackhi_epi64(t9, t11);
    let t12 = _mm_unpacklo_epi32(v12_hi, v13_hi); let t13 = _mm_unpackhi_epi32(v12_hi, v13_hi);
    let t14 = _mm_unpacklo_epi32(v14_hi, v15_hi); let t15 = _mm_unpackhi_epi32(v14_hi, v15_hi);
    let r12 = _mm_unpacklo_epi64(t12, t14); let r13 = _mm_unpackhi_epi64(t12, t14);
    let r14 = _mm_unpacklo_epi64(t13, t15); let r15 = _mm_unpackhi_epi64(t13, t15);
...
mod tests {
    ...
    fn test_avx2_produces_correct_output() {
        ...
        assert_eq!(&avx2_output[0..64], &scalar_output[..], "First block mismatch:\nAVX2:   {:02x?}\nScalar: {:02x?}", &avx2_output[0..64], &scalar_output[..]);
        ...
        scalar_serialize_state(&block, &mut scalar_output);
        assert_eq!(&avx2_output[64..128], &scalar_output[..], "Second block mismatch:\nAVX2:   {:02x?}\nScalar: {:02x?}", &avx2_output[64..128], &scalar_output[..]);
        ...
        scalar_serialize_state(&block, &mut scalar_output);
        assert_eq!(&avx2_output[128..192], &scalar_output[..], "Third block mismatch");
        ...
        scalar_serialize_state(&block, &mut scalar_output);
        assert_eq!(&avx2_output[192..256], &scalar_output[..], "Fourth block mismatch");
    }
