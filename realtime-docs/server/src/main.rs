...
async fn list_documents(State(state): State<AppState>) -> Result<Json<ListResponse>, StatusCode> {
    ...
    .await
    .map_err(|err| {
        tracing::error!("Failed to list documents: {}", err);
        StatusCode::INTERNAL_SERVER_ERROR
    })?;

    let documents: Vec<DocumentInfo> = rows
        .into_iter()
        .map(|(id, title, created_at, updated_at)| DocumentInfo {
            id,
            title,
            created_at,
            updated_at,
...
async fn create_document(
    ...
    doc.put(automerge::ROOT, "id", id.clone()).map_err(|err| {
    ...
    tracing::info!("Created document: {} ({})", payload.title, id);
...
async fn delete_document(
    ...
    match result {
        ...
        Err(err) => {
            tracing::error!("Failed to delete document {}: {}", id, err);
            StatusCode::INTERNAL_SERVER_ERROR
        }
    }
}
...
async fn sync_document(
    ...
    let title = if let Ok(doc) = automerge::AutoCommit::load(&merged_binary) {
        doc.get(automerge::ROOT, "title")
            ...
            .unwrap_or_else(|| format!("Document {}", &id[..8.min(id.len())]))
    } else {
        format!("Document {}", &id[..8.min(id.len())])
    };

    // Upsert the document
    sqlx::query(
        r#"
        INSERT INTO documents (id, title, document, created_at, updated_at)
        VALUES (?, ?, ?, ?, ?)
    ...
    .await
    .map_err(|err| {
        tracing::error!("Failed to upsert document {}: {}", id, err);
        StatusCode::INTERNAL_SERVER_ERROR
    })?;
...
async fn handle_socket(socket: WebSocket, document_id: String, state: AppState) {
    ...
    tracing::info!("WebSocket connection closed for document {}", document_id);
}

async fn broadcast_update(state: &AppState, document_id: &str, binary: &[u8], sender_id: &str) {
