...
async fn start_test_server_with_websocket() -> (String, u16, TempDir, MockServer) {
    ...
    tokio::spawn(async move {
        ...
        let llm_client: Arc<dyn ai_coding_agent_server::llm::LlmClient> = Arc::new(
            ai_coding_agent_server::llm::OpenAiLlmClient::new(llm_config),
        );

        let templates = Arc::new(tokio::sync::RwLock::new(
            ai_coding_agent_server::templates::TemplateManager::new(),
        ));
        let orchestrator = Arc::new(ai_coding_agent_server::orchestrator::Orchestrator::new(
            llm_client.clone(),
        ));
        ...
        let app = Router::new()
            .route(
                "/api/sessions",
                post(ai_coding_agent_server::handlers::sessions::create_session),
            )
            .route(
                "/api/sessions",
                get(ai_coding_agent_server::handlers::sessions::list_sessions),
            )
            .route(
                "/api/sessions/:id",
                get(ai_coding_agent_server::handlers::sessions::get_session),
            )
            .route(
                "/api/sessions/:id/messages",
                post(ai_coding_agent_server::handlers::sessions::send_message),
            )
            .route(
                "/api/sessions/:id/messages",
                get(ai_coding_agent_server::handlers::sessions::get_messages),
            )
            .route(
                "/api/sessions/:id/stream",
                get(ai_coding_agent_server::handlers::websocket::session_stream),
            )
    ...
    assert!(
        wait_for_server(&base_url, 20).await,
...
async fn test_e2e_session_isolation() {
    ...
    let messages1: Vec<serde_json::Value> =
        messages_response1.json().await.expect("Failed to parse");
    ...
    let messages2: Vec<serde_json::Value> =
        messages_response2.json().await.expect("Failed to parse");
...
async fn test_e2e_complete_websocket_workflow() {
    ...
    let ws_url = format!("ws://127.0.0.1:{}/api/sessions/{}/stream", port, session_id);
    let (ws_stream, _) = connect_async(&ws_url).await.expect(
        "Failed to connect to WebSocket - this indicates the block_on panic issue is still present",
    );
    ...
    tokio::spawn(async move {
        ...
        let _ = client
            .post(format!(
                "{}/api/sessions/{}/messages",
                base_url_clone, session_id_clone
            ))
    ...
    assert!(
        !messages.is_empty(),
        "Should have at least the user message stored"
    );
}
...
async fn test_e2e_websocket_subscribe_creates_session_state() {
    ...
    assert!(
        result.is_ok(),
        "Should be able to connect to WebSocket for new session"
    );
}
